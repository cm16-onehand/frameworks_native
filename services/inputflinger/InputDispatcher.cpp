...
sp<InputWindowHandle> InputDispatcher::findTouchedWindowAtLocked(int32_t displayId,
    ...
    for (size_t i = 0; i < numWindows; i++) {
        ...
        if (windowHandle->getName().find("SingleMode_windowbg") != std::string::npos) {
            bool leftSingleHandScreen = (windowHandle->getName().find("left") != std::string::npos);
...
int32_t InputDispatcher::findTouchedWindowTargetsLocked(nsecs_t currentTime,
    ...
    for (size_t i = 0; i < numWindows; i++) {
        ...
        const InputWindowInfo* windowInfo = windowHandle->getInfo();
        //int32_t flags = windowInfo->layoutParamsFlags;
        if (windowInfo->visible && windowHandle->getName().find("SingleMode_windowbg_hint") != std::string::npos) {
            hint = true;
            break;
        }
    }
    for (size_t i = 0; (!hint && i < numWindows); i++) {
        ...
        const InputWindowInfo* windowInfo = windowHandle->getInfo();
        //int32_t flags = windowInfo->layoutParamsFlags;
        if (windowInfo->visible && windowHandle->getName().find("SingleMode_windowbg") != std::string::npos ){
...
bool InputDispatcher::isWindowObscuredAtPointLocked(
    ...
    for (size_t i = 0; i < numWindows; i++) {
        ...
        if (otherHandle->getName().find("SingleMode_windowbg") != std::string::npos) {
            bool leftSingleHandScreen = (otherHandle->getName().find("left") != std::string::npos);
            int32_t origin_xx = (int32_t)((float)x * SINGLE_HAND_SCALE);
            int32_t origin_yy = (int32_t)((float) y * SINGLE_HAND_SCALE + (float)dinfo.h * (1-SINGLE_HAND_SCALE));
            if (!leftSingleHandScreen) {
                origin_xx += (int32_t)((float)dinfo.w * (1-SINGLE_HAND_SCALE));
...
void InputDispatcher::singleHandModePreProcessLocked(MotionEntry* entry) {
    ...
    for (size_t i = 0; i < numWindows; i++) {
        ...
        if (windowInfo->visible && windowHandle->getName().find("SingleMode_windowbg") != std::string::npos) {
            hasVirtualDevice = true;
            leftSingleHandScreen = (windowHandle->getName().find("left") != std::string::npos);
            break;
        }
    }
    if (!hasVirtualDevice) {
